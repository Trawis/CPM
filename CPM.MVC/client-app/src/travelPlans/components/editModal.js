...
import { Textbox, Selectbox, mapFromSelectbox, mapToSelectbox } from '../../shared';
import { API } from '../../core';
import "imrc-datetime-picker/dist/imrc-datetime-picker.css";

export const EditModal = ({ isShowing, hide, travelPlan, getAll, carOptions, employeeOptions }) => {
    ...
    useEffect(() => {
        ...
        setTravelPlanState({
            ...travelPlan,
            startDate: moment(travelPlan.startDate).format('MM/DD/YYYY HH:mm'),
            endDate: moment(travelPlan.endDate).format('MM/DD/YYYY HH:mm'),
            car: mapToSelectbox(travelPlan.car, "name", "carId"),
            employees: mapToSelectbox(travelPlan.employees, "name", "employeeId"),
        });
    }, [travelPlan]);
    ...
    const editTravelPlan = async () => {
        try {
            ...
            await API.travelPlan.update(travelPlan).then(getAll).then(hide);
        } catch (e) {
            if (e.errors) {
    ...
    return (
        <Modal size="lg" show={isShowing} onHide={hide} backdrop="static" centered>
            ...
            <Modal.Footer className="modal-footer d-inline">
                <Button variant="primary" className="pull-left" onClick={editTravelPlan}><FontAwesomeIcon icon={faSave} /> Save</Button>
